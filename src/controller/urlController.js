...
const Validation = require('../validation/urlValidation')
const { createClient } = require("redis");

// Redis v5 - optional cache (app works without it)
...
async function initRedis() {
  try {
    ...
    console.log("⚠️ Redis not available - running without cache:", err.message);
  }
}
...
const createShortUrl = async function (req, res) {
    try {
        ...
        let checkLongUrl = await redisGet(`${longUrl}`)
        if(checkLongUrl){
            return res.status( 400).send({
                status: false,
        ...
        // to set the created shorturl in cache memory (for redirect + already-shortened check)
        await redisSet(urlCode, longUrl)
        await redisSet(longUrl, JSON.stringify({ longUrl, shortUrl, urlCode }))
...
const redirectLongUrl = async function (req, res) {
    ...
    try {
        ...
        let cachedProfileData = await redisGet(`${urlCode}`)
        if (cachedProfileData) {
            res.status(302).redirect(cachedProfileData)
        } else {
            ...
            await redisSet(`${urlCode}`, checkUrlCode.longUrl)
